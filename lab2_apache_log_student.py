...
def parseLogs():
    ...
    parsed_logs = (sc
                   .textFile('apache.access.log.PROJECT')
...
topTenErrURLs = endpointSum.takeOrdered(10, lambda a:-1 * a[1])
print 'Top Ten failed URLs: %s' % topTenErrURLs
print 'total failed endpoints: %s' % endpointSum.count()
...
hosts = access_logs.map(lambda a:a.host)
...
dayToHostPairTuple = access_logs.map(lambda a:(a.date_time.day, a.host))
print '0: %s' % dayToHostPairTuple.count()

dayGroupedHosts =  dayToHostPairTuple.distinct()
print("1: %s %s", dayGroupedHosts.take(5),dayGroupedHosts.count())

dayHostCount = dayGroupedHosts.map(lambda a:(a[0],1))
print '2: %s' % dayHostCount
...
dayAndHostTuple = access_logs.map(lambda a:(a.date_time.day, 1))
print 'dayAndHostTuple: %s' % dayAndHostTuple.take(10)

groupedByDay = dayAndHostTuple.reduceByKey(lambda a,b:a+b)
print 'groupedByDay: %s' % groupedByDay.take(10)

sortedByDay = groupedByDay.sortByKey(True)
print 'sortedByDay: %s' % sortedByDay.take(30)

joinedUniqueAndTotal = sortedByDay.join(dailyHosts)
print 'joinedUniqueAndTotal ', joinedUniqueAndTotal

avgDailyReqPerHost = (joinedUniqueAndTotal                      .map(lambda a:(a[0],(a[1][0]//a[1][1])))                      .sortByKey()                     .cache())
...
badEndpoints = badRecords.map(lambda a:a.endpoint)
...
errDateCountPairTuple = badRecords.map(lambda record:(record.date_time.day,1))
...
topErrDate = errDateSorted.takeOrdered(5, lambda a: -1*a[1])
...
hourCountPairTuple = badRecords.map(lambda record:(record.date_time.hour,1))
