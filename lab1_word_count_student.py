...
# TODO: Replace <FILL IN> with appropriate code
wordCountsCollected = (wordsRDD.map(lambda a:(a,1)).reduceByKey(lambda a,b:a+b).collect())
...
# TODO: Replace <FILL IN> with appropriate code
uniqueWords = len(wordsGrouped.collect())
print uniqueWords
...
totalCount = wordCounts.map(lambda a:a[1]).reduce(lambda a,b:a+b)
...
def removePunctuation(text):
    ...
    return string.strip(text).lower()
...
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda a:a.split(u' '))
#print tempRDD.top(30)
#tempRDD2 = tempRDD.flatMap(lambda a:(u' ' if a==u'' else a).split(u' '))
#shakespeareWordsRDD = tempRDD2.flatMap(lambda a:re.sub(pattern2,u' ',a).split(u' '))
#shakespeareWordsRDD = tempRDD.map(lambda a:a if len(a)>=0 )
#shakespeareWordsRDD = tempRDD.map(lambda a:a.collect())
