...
def simpleTokenize(string):
    ...
    #temp = string.strip(text).lower()
    lowered = string.lower()
    tokenized = re.compile(split_regex).findall(lowered)
    return tokenized
...
def tokenize(string):
    ...
    """
    new_list = []
    simpleTokenizedString = simpleTokenize(string)
    for token in simpleTokenizedString:
...
def countTokens(vendorRDD):
    ...
    """
    temp = vendorRDD.map(lambda a:len(a[1])).reduce(lambda a,b: a+b)
...
def findBiggestRecord(vendorRDD):
    ...
    """
    #temp = vendorRDD.map(lambda a:(a[0],a[1],len(a[1]))).takeOrdered
    #print temp[0]
    #bigRecord = temp.map(lambda a:((a[1],a[0]),)).take(1)
    
    bigRecord = vendorRDD.map(lambda a:(len(a[1]),(a[0],a[1]))).sortByKey(False,1)
    #print bigRecord.take(1)
    bigSorted = bigRecord.map(lambda a:(a[1][0],a[1][1])).take(1)
    #print bigSorted
...
def tf(tokens):
    ...
    for key in tokendict:
        tokendict[key] = tokendict[key]/float(tokenCount)
    return tokendict
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    #print('0: ', corpus.collect())
    uniqueTokens = corpus.flatMap(lambda record:set(record[1]))
    #print('1: ', uniqueTokens.collect())
    tokenCountPairTuple = uniqueTokens.map(lambda record:(record,1))
    tokenSumPairTuple = tokenCountPairTuple.reduceByKey(lambda a,b: a+b)
    print('2: ', tokenSumPairTuple.take(2))
    return (tokenSumPairTuple.map(lambda record:(record[0],N/float(record[1]))))
...
def dotprod(a, b):
    ...
    """for k,v in a.iteritems():
        if k in b.keys():
            product = product + v * b[k]"""
    product = sum(a[k] * b[k] for k in keys)
...
def norm(a):
    ...
    """
    keys = a.viewkeys()
    self_product = sum(a[k] * a[k] for k in keys)
            
   # print('self product ', self_product)
    sqrt = math.sqrt(self_product)
...
def cossim(a, b):
    ...
    """
    similarity = dotprod(a,b)/(norm(a)*norm(b))
...
print('sims ', sims.take(2))

trueDupsRDD = (sims
               .join(goldStandard)
               .map(lambda rec: rec[1][0]))

print('true dups ', trueDupsRDD.take(2))
      
trueDupsCount = trueDupsRDD.count()

avgSimDups = sum(trueDupsRDD.collect())/float(trueDupsCount)
print('avg dups ', avgSimDups)

nonDupsRDD = (sims
              .fullOuterJoin(goldStandard)
            .filter(lambda rec: (rec[1][0] == None or rec[1][1] == None))
             .map(lambda rec: rec[1][0])
             )
print('non dups ', nonDupsRDD.take(30))

nonDupsCount = nonDupsRDD.count()
print('non dups count ', nonDupsCount)

avgSimNon = sum(nonDupsRDD.filter(lambda rec:rec!=None).collect())/float(nonDupsCount)
...
#print('broadcast variable ', idfsFullBroadcast.value)
...
commonTokens = (amazonInvPairsRDD                .join(googleInvPairsRDD)                .map(lambda rec:swap(rec))                .groupByKey()
                .cache())
...
print '1' ,amazonWeightsBroadcast.value['b00005lzly']
print '2' ,commonTokens.take(1)
print '3' ,amazonWeightsBroadcast.value['b00005lzly']
